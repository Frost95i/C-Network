...
socket_wrapper::Socket accept_client(socket_wrapper::Socket& server_sock)
{
    ...
    std::cout <<
        "Client: " << inet_ntop(AF_INET, &(reinterpret_cast<const sockaddr_in* const>(&client_addr)->sin_addr), &addr[0], addr.size())
        << "..."
        << std::endl;
    return client_sock;
}
...
int main(int argc, const char * const argv[])
{
    ...
    try
    {
        ...
        if (!servinfo)
        {
            std::cerr << "Servinfo empty" << std::endl;
            exit(EXIT_FAILURE);
        }
